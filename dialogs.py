...
def do_options_dlg():
    ...
    while True:
        ...
        text = '\n'.join([]
            ...
            +[c1.join(['type=check', 'pos=6,100,400,0', 'cap='+_('Use all 3 colors from syntax-theme'), 'val='+str(int(opt.colors_themed))])]
            +[c1.join(['type=check', 'pos=6,130,400,0', 'cap='+_('Lint on &opening file'), 'val='+str(int(use_on_open))])]
            +[c1.join(['type=check', 'pos=6,156,400,0', 'cap='+_('Lint on &saving file'), 'val='+str(int(use_on_save))])]
            +[c1.join(['type=check', 'pos=6,182,400,0', 'cap='+_('Lint &after text changed, and pause'), 'val='+str(int(use_on_change))])]
            +[c1.join(['type=button', 'pos=206,212,300,0', 'cap='+_('OK'), 'ex0=1'])]
            +[c1.join(['type=button', 'pos=306,212,400,0', 'cap='+_('Cancel')])]
                        )
        res = app.dlg_custom(_('CudaLint options'), 406, 245, text)
    ...
    opt.colors_themed = text[id_themed]=='1'
